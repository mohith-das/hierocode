...
def build_config_yaml(detection: DetectionResult) -> str:
    ...
    # Emit exploration field only when user explicitly opted in AND provider supports it.
    _active_providers = {"claude_code_cli", "codex_cli"}
    emit_exploration = (
        detection.active_exploration and planner_type in _active_providers
    )
    exploration_suffix = "\n    exploration: active" if emit_exploration else ""

    auto_apply_line = "\n  auto_apply: true" if detection.auto_apply else ""
...
routing:
  planner:
    ...
    model: {planner_model}{exploration_suffix}
  drafter:
...
policy:
  ...
  warn_before_escalation: true{auto_apply_line}
...
def run_wizard(write: bool = True, force: bool = False) -> tuple[DetectionResult, str]:
    ...
    if sys.stdin.isatty() and detection.recommended_planner_type in ("claude_code_cli", "codex_cli"):
        ans = input(
            "Enable active exploration for the planner? Claude/Codex will use\n"
            "Read/Grep/Glob tools during planning & QA — better plans but higher\n"
            "token cost and more of your subscription quota. [y/N]: "
        ).strip().lower()
        detection.active_exploration = ans == "y"

    if sys.stdin.isatty():
        ans = input(
            "Auto-apply drafter output to disk without per-file confirmation? "
            "Trades safety for speed — OK for low-stakes work, not recommended for "
            "shared codebases. [y/N]: "
        ).strip().lower()
        detection.auto_apply = ans == "y"
